...
const ITEMS_DEFAULT = [{ tipo: 'mano_obra', descripcion: '', unidad: 'u', cantidad: 1, precio_unit: 0 }]

export default function NuevoPresupuesto() {
  const navigate = useNavigate()
  const { crear, loading: saving } = usePresupuestos()
  const { clientes, crear: crearCliente } = useClientes()

  const [step, setStep] = useState(1) // 1=cliente, 2=items, 3=resumen
  ...
  function setItem(idx, key, val) {
    setItems(prev => prev.map((it, i) => i === idx ? { ...it, [key]: key === 'cantidad' || key === 'precio_unit' ? Number(val) || 0 : val } : it))
  }
  function addItem(tipo) {
    setItems(prev => [...prev, { tipo, descripcion: '', unidad: tipo === 'material' ? 'u' : 'global', cantidad: 1, precio_unit: 0 }])
  }
    ...
    <div className="flex-1 overflow-y-auto pb-32" style={{ background: '#0D0D14' }}>
      ...
      <div className="flex items-center gap-3 px-4 pt-12 pb-4 sticky top-0 z-10" style={{ background: '#0D0D14' }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-[12px]">Paso {step} de 3</p>
        </div>
        {/* steps */}
        ...
        <div className="px-4 flex flex-col gap-4">
          ...
          {modoCliente === 'existente' ? (
            <div className="flex flex-col gap-2">
              {clientes.length === 0
                ? <p className="text-gray-500 text-[13px] text-center py-4">No tenés clientes aún. Creá uno nuevo.</p>
                : clientes.map(c => (
          ...
          {/* vigencia */}
          <div>
            <label className="text-gray-500 text-[11px] mb-1 block">Vigencia del presupuesto</label>
            <div className="flex gap-2">
              {[3, 5, 7, 10, 15].map(d => (
                <button key={d} onClick={() => setVigencia(d)}
                  className="flex-1 py-2.5 rounded-xl text-[12px] font-semibold"
                  style={{
                    background: vigencia === d ? '#3B82F6' : '#161622',
                    color: vigencia === d ? '#fff' : '#6B7280',
                    border: vigencia === d ? 'none' : '1px solid #1E1E2E',
                  }}>
                  {d}d
                </button>
              ))}
          ...
          <button onClick={() => setStep(2)}
            ...
            className="w-full py-4 rounded-2xl text-white font-bold text-[15px] mt-2"
            style={{ background: '#3B82F6' }}>
        ...
        <div className="px-4 flex flex-col gap-4">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="rounded-2xl p-4" style={{ background: '#161622', border: '1px solid #1E1E2E' }}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex gap-2">
                  {[['material', '🔧 Material'], ['mano_obra', '👷 Mano de obra']].map(([k, l]) => (
                ...
                <button onClick={() => removeItem(idx)} className="text-red-500/60">
                  <Trash2 size={16} />
                </button>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-[2]">
                  ...
                  <input type="number" value={item.precio_unit || ''}
          ...
          <div>
            ...
            <textarea value={notas} onChange={e => setNotas(e.target.value)}
              rows={3} placeholder="Observaciones, acuerdos con el cliente..."
        ...
        <div className="px-4 flex flex-col gap-4">
          ...
          <div className="rounded-2xl p-5 text-center" style={{ background: '#161622', border: '1px solid #1E1E2E' }}>
            ...
            <div className="flex justify-center gap-6 mt-3">
              ...
              <div>
                ...
                <p className="text-green-400 font-bold text-[14px]">{fmt(margen)}</p>
              </div>
            </div>
          </div>

          <div className="rounded-2xl p-4 flex flex-col gap-2" style={{ background: '#161622', border: '1px solid #1E1E2E' }}>
            <p className="text-gray-500 text-[11px] font-semibold tracking-wider mb-1">ÍTEMS ({items.filter(i => i.descripcion).length})</p>
          ...
          <div className="flex flex-col gap-3 mt-2">
            ...
            <button onClick={() => guardar('enviado')} disabled={guardando}
              className="w-full py-4 rounded-2xl text-white font-bold text-[15px] flex items-center justify-center gap-2"
              style={{ background: '#3B82F6' }}>
              {guardando
